...
EASTER_EGG_CREATURES = {
    # FOG creatures - eldritch horrors lurking in the mist
    "fog_night": [
        {
            ...
            "colour": "MAGIC",
            "speed": 0.15,
            "rarity": 0.0004,  # Rare - roughly every few minutes
        },
        {
            "name": "Eldritch Eye",
            "frames": [
...
class EasterEggManager:
    """Manages rare creature appearances."""
    ...
    def get_creature_key(self, condition, hour: int) -> str:
        """Get the appropriate creature category."""
        ...
        elif condition in (WeatherCondition.RAIN, WeatherCondition.HEAVY_RAIN, WeatherCondition.DRIZZLE):
        ...
        elif condition in (WeatherCondition.CLOUDY, WeatherCondition.PARTLY_CLOUDY):
            return "cloudy_any"
        return None
    
    def try_spawn(self, condition, hour: int):
        """Attempt to spawn a creature based on rarity."""
        if self.active_creature:
            return  # Already have one
        
        key = self.get_creature_key(condition, hour)
        if not key or key not in EASTER_EGG_CREATURES:
            return
        
        creatures = EASTER_EGG_CREATURES[key]
    ...
    def draw(self, screen, lightning_active: bool = False):
        ...
        colour = getattr(Theme, colour_name, Theme.SNOW)
        if lightning_active:
            colour = Theme.SUN
        
        # Draw creature
        x = int(self.creature_x)
        y = random.randint(6, self.height - len(frame) - 4) if not hasattr(self, '_y') else self._y
...
EASTER_EGG_CREATURES = {
    ...
    # THUNDERSTORM creatures - dragons and storm spirits
    "storm_night": [
        {
            "name": "Dragon",
            "frames": [
